...
def retrieve_from_knowledge_base(kb_id: str, query: str, top_k: int = 3) -> list[dict]:
    """
    ...
    try:
        ...
        return [
            {"text": f"Knowledge base retrieval failed: {exc}", "source": "error", "score": 0.0}
        ]
    ...
    for item in response.get("retrievalResults", []):
        ...
        results.append({"text": text, "source": source, "score": round(float(score), 4)})
