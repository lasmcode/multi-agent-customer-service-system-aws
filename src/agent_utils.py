...
def _strip_xml_tags(text: str) -> str:
    ...
    text = re.sub(
        r"<(search_quality_reflection|search_quality_score)>.*?</\1>", "", text, flags=re.DOTALL
    )
...
class _TraceWriter:
    ...
    def _emit(self, line: str) -> None:
        ...
        if m:
            ...
            self._real.write(f"  {_C.GRY}  [TOOL CALL]  " f"{_C.RESET}{_C.BOLD}{tool}{_C.RESET}\n")
        else:
            self._real.write(f"  {_C.DIM}  | {s}{_C.RESET}\n")
...
class AgentTrace:
    ...
    def new_turn(self) -> None:
        ...
        _trace_print(
            f"  {_C.ORCH}{_C.BOLD}[ORCHESTRATOR]{_C.RESET}  "
    ...
    def summary(self, session_id: str, elapsed: float) -> None:
        ...
        _trace_print(f"  {_C.GRY}{'=' * w}{_C.RESET}")
        _trace_print(
            f"  {_C.BOLD}WORKFLOW STATE SUMMARY{_C.RESET}  "
    ...
    def step_start(self, column: str) -> None:
        ...
        _, label, colour, role = _AGENT_META.get(column, ("[AGT]", column.upper(), _C.GRY, ""))
        n = self._step
        w = _C.W
        _trace_print()
        _trace_print(f"  {_C.GRY}{'─' * w}{_C.RESET}")
        _trace_print(
            f"  {_C.GRY}[ STEP {n:02d} ]{_C.RESET}  " f"{colour}{_C.BOLD}{label}{_C.RESET}"
        )
    ...
    def step_done(self, column: str, old_version: int) -> None:
        ...
        new_v = old_version + 1
        _trace_print(
            f"  {_C.GRY}           Status : {_C.OK}[COMPLETE]{_C.RESET}  "
    ...
    def kb_start(self, kb_map: dict) -> None:
        ...
        _trace_print(
            f"  {_C.GRY}  [PARALLEL RETRIEVAL - START]  "
        ...
        _trace_print(
            f"  {_C.DIM}  Retrieving in parallel... "
    ...
    def kb_done(self, count: int) -> None:
        ...
        _trace_print(f"  {_C.GRY}  {'─' * 54}{_C.RESET}")
        _trace_print(
            f"  {_C.GRY}  [PARALLEL RETRIEVAL - END]  "
    ...
    def kb_result(self, domain: str, content: str) -> None:
        ...
        _trace_print()
        _trace_print(
            f"  {_C.GRY}  [{num}]  " f"{_C.KB}{label}{_C.RESET}  " f"{_C.GRY}KB: {kb_id}{_C.RESET}"
        )
