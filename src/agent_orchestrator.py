...
def _update_workflow_state(
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(f"WorkflowState version conflict on attempt {attempt+1}, retrying...")
...
def _gw_get_or_create(
    ...
    try:
        gw = agentcore_ctrl.create_gateway(
            ...
            protocolType="MCP",
            authorizerType="NONE",
            protocolConfiguration={"mcp": {"instructions": instructions, "searchType": "SEMANTIC"}},
        )
        gw_id = gw["gatewayId"]
...
def _gw_create_target(agentcore_ctrl, gateway_id: str, t: dict, lambda_arn: str) -> None:
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "deploy":
        ...
        customer_id = input(f"  Enter Customer ID (default: CUST-001): ").strip() or "CUST-001"
        session_id = str(uuid.uuid4())[:8]
        print()
        ...
        while True:
            ...
            prompt = f"[Session ID: {session_id}] " f"[Customer ID: {customer_id}] {user_input}"
            t0_turn = time.time()
